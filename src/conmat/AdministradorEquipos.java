...
public class AdministradorEquipos extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        ImagenEquipo.setBackground(new java.awt.Color(255, 255, 255));
        ImagenEquipo.setForeground(new java.awt.Color(255, 255, 255));
        ImagenEquipo.setBorder(new javax.swing.border.LineBorder(new java.awt.Color(102, 102, 102), 1, true));
        ImagenEquipo.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                ImagenEquipoMouseClicked(evt);
            }
        });
        jPanel2.add(ImagenEquipo, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 80, 210, 220));
    ...
    private void ImagenEquipoMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_ImagenEquipoMouseClicked
        javax.swing.JFileChooser ventana = new javax.swing.JFileChooser();
        ventana.setDialogTitle("Buscar Foto del Equipo");
        int opcion = ventana.showOpenDialog(this);
        if (opcion == JFileChooser.APPROVE_OPTION) {
            String fil = ventana.getSelectedFile().getPath();
            RSScaleLabel.setScaleLabel(ImagenEquipo, ventana.getSelectedFile().toString());
            rutaimagen = ventana.getSelectedFile().toString();
        }
    }//GEN-LAST:event_ImagenEquipoMouseClicked
    ...
    private javax.swing.JLabel ImagenEquipo;
    ...
    private void CargaImagen(String IdEquipo) {
        ConexionDb db = new ConexionDb();
        Statement st = db.Conecxion();
        ResultSet consulta;

        try {
            consulta = db.realizarConsulta("SELECT ID,IMAGEN FROM EQUIPOS WHERE ID = " + IdEquipo, st);
            consulta.next();
            String imgst = consulta.getString("IMAGEN");
            if (imgst == null) {
                ImagenEquipo.setIcon(icon);
            } else {
                Blob imagenproducto = consulta.getBlob("IMAGEN");
                ...
                Image imagencompleta = imgproducto.getScaledInstance(ImagenEquipo.getWidth(), ImagenEquipo.getHeight(), Image.SCALE_SMOOTH);
                ImagenEquipo.setIcon(new ImageIcon(imagencompleta));
            }
